...
public class SingleValueSummaryTab extends JPanel {
    ...
    private G2dPanel chartContainerPanel;
    ...
    private void updateFindButtonState() {
        ...
        if (isAcrossTime) {
            toggleButton.setToolTipText(showingChart ? "Switch to text view" : "Switch to chart view");
        } else {
    ...
    private void findEnsemble() {
        try {
            ...
            // Step 1: compute one value per ensemble member across time.
            // CUMULATIVE produces one iteration per day value; other stats produce one.
            ...
            // Step 2: for each step 1 result, find the nearest member.
            // PERCENTILES produces one iteration per percentile; other stats produce one.
            float[] step2Entries = (stat2 == Statistics.PERCENTILES) ? stat2Values : new float[]{0};
            ...
            for (int s1 = 0; s1 < step1Iterations.length; s1++) {
                ...
                for (float entry : step2Entries) {
                    ...
                    String statLabel = (stat2 == Statistics.PERCENTILES)
                            ? String.format("%.2f%% Percentile", entry * 100)
                            : stat2.toString();
    ...
    private void buildChart() {
        ...
        // Add ensemble member lines (deferred — view is null until generateChart)
        ...
        // Swap the new chart into the existing G2dPanel container (TimeSeriesTab pattern)
    ...
    private void organizeUI() {
        ...
        // Pre-build an empty G2dPanel as the chart card so it's in the visible hierarchy
        // from the start (mirrors TimeSeriesTab). Without this, G2dPanel.buildComponents
        // runs against a zero-sized parent on first toggle and the chart never paints.
        chartContainerPanel = new EnsembleChartAcrossTime().generateChart();
